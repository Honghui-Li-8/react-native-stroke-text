...
config.resolver.extraNodeModules = {
  '@charmy.tech/react-native-stroke-text': monorepoRoot,
};
...
config.resolver.resolveRequest = (context, moduleName, platform) => {
  if (moduleName === '@charmy.tech/react-native-stroke-text') {
    // Prefer source file
    const sourceFile = path.join(monorepoRoot, 'src', 'index.tsx');
  ...
  if (moduleName.includes('StrokeTextViewNativeComponent')) {
    const sourceFile = path.join(monorepoRoot, 'src', 'StrokeTextViewNativeComponent.ts');
    if (fs.existsSync(sourceFile)) {
      return {
        type: 'sourceFile',
        filePath: sourceFile,
